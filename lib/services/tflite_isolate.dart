...
/// Minimal isolate worker that loads a TFLite model inside the isolate
/// and accepts raw input buffers to run inference. The protocol is:
/// - main sends: {@code {'cmd':'load','model':'assets/my.tflite'}}
/// - main sends: {@code {'cmd':'run','data':List<double>}}
/// - worker replies with result list.
    ...
    port.listen((message) async {
      try {
        ...
        if (cmd == 'load') {
          ...
          // Build input and output based on interpreter tensors
          try {
            final input = [data];
